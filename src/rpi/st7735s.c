...
static uint8_t init_cmd[] = {
    ...
    2, MADCTL, 0b00100000, /* row oder, col order, row colum xchange, vert refr order, rgb/bgr, hor refr order, 0, 0 */
...
void ST7735S_flush(void)
{
    ...
    ST7735S_addrset(xm, ym, xx, yx);

/* TODO: deprecate, to use ST7735S_addrset
...
{
    uint8_t cas[] = {CASET, XS >> 8, XS, XE >> 8, XE};
    SPI_Transmit(sizeof(cas), cas);

    uint8_t ras[] = {RASET, YS >> 8, YS, YE >> 8, YE};
    SPI_Transmit(sizeof(ras), ras);

    uint8_t ram[] = {RAMWR};
    SPI_Transmit(1, ram); // Memory write
}